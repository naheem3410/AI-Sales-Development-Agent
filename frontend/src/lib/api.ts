...
async function apiFetchFormData<T>(path: string, token: string, form: FormData): Promise<T> {
  const url = `${config.api.baseUrl}${path}`
...
// ── API factory — call with token from useAuth() ───────────────────────────────

export function createApiClient(token: string) {
  const get  = <T>(path: string) => apiFetch<T>(path, token)
  const post = <T>(path: string, body?: unknown) =>
    apiFetch<T>(path, token, { method: 'POST', body: body ? JSON.stringify(body) : undefined })
  const put  = <T>(path: string, body?: unknown) =>
    apiFetch<T>(path, token, { method: 'PUT',  body: body ? JSON.stringify(body) : undefined })
  const patch = <T>(path: string, body?: unknown) =>
    apiFetch<T>(path, token, { method: 'PATCH', body: body ? JSON.stringify(body) : undefined })
  const del  = <T>(path: string) =>
    apiFetch<T>(path, token, { method: 'DELETE' })
  const postForm = <T>(path: string, form: FormData) =>
    apiFetchFormData<T>(path, token, form)
